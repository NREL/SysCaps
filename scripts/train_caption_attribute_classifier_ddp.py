...
if __name__ == '__main__':
    ...
    if args.rank == 0 and not checkpoint_dir.exists():
        os.makedirs(checkpoint_dir)

    resstock_comstock = 'comstock' if args.dataset == 'energyplus_comstock' else 'resstock'
    attributes = open(BUILDINGS_BENCH_PATH / 'metadata' / f'attributes_{resstock_comstock}.txt', 'r').read().split('\n')
    df1 = pd.read_parquet(BUILDINGS_BENCH_PATH / 'metadata' / f"{resstock_comstock}_amy2018.parquet", engine="pyarrow")
    df2 = pd.read_parquet(BUILDINGS_BENCH_PATH / 'metadata' / f"{resstock_comstock}_tmy3.parquet", engine="pyarrow")
    ...
    if args.rank == 0:
        if args.resume_from_checkpoint != '' and not args.evaluate:
            run = wandb.init(
    ...
    if args.resume_from_checkpoint != '':
        model, optimizer, _, batch_step , best_val_loss, patience_counter = utils.load_model_checkpoint(
            checkpoint_dir / args.resume_from_checkpoint, model, local_rank, optimizer)
        print(f'successfully loaded model checkpoint {args.resume_froqm_checkpoint}...')
    ...
    if not args.evaluate:
        for epoch in range(starting_epoch, args.max_epochs):
            ...
            for batch in tqdm(train_dataloader):
                ...
                if args.rank == 0 and batch_step % 50 == 0:
                ...
                if args.rank == 0 and batch_step % 500 == 0:
    ...
    if args.rank == 0:
        ...
        for k,v in attribute_results.items():
            v_ = np.mean(v)
            print(f'attribute {k} pred. accuracy (%) :: {100*v_:.3f}')
